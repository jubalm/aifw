...
const { copyTemplate, promptUser, checkExistingFiles } = require('./utils.js');
...
async function init(claudeCodeFlag = false) {
  ...
  if (existingFiles.length > 0) {
    ...
    const overwrite = await promptUser('Continue and overwrite existing files? (y/n): ');
  ...
  try {
    ...
    if (platform === 'claude-code') {
      ...
      console.log('     claude --agent llm-generator "Generate project context"');
      console.log('  3. Create specialized agents:');
      console.log('     claude --agent agent-builder "Create specialized agents"');
    }
